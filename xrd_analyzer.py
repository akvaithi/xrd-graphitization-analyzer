...
class GraphitizationAnalyzer:
    ...
    def _run_b(self) -> dict:
        ...
        Ag, xcg, wg, mug, At, xct, wt, mut = self._fit(
            _dual_model, x, y, p0_d, (lo_d, hi_d), "Method B dual-peak")
...
_CSV_COLUMNS = (
    ...
    "turbostratic_xc", "DG_single_percent", "dg_overestimation_percent",
    "crystallite_height_Lc_angstrom", "error",
)
...
def _print_human(result: dict, filepath: str | Path) -> None:
    ...
    if result["method"] == "A":
        ...
        print(f"\n  DG% (legacy single)   : {result['DG_single_percent']:.2f} %")
        print(f"  DG% overestimation    : {result['dg_overestimation_percent']:.2f} %  "
              "(legacy − NETL)")
