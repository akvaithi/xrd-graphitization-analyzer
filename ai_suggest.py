...
5. SPECIMEN DISPLACEMENT (sample-height error): a well-crystallized graphite (002) sits at ~26.50-26.60 (Cu K-alpha). IMPORTANT: if internal_standard_offset_applied_deg is nonzero, the 2-theta scale has ALREADY been calibrated against residual internal_standard_phase peaks (a reliable measured correction) - then set displacement_suspected=false and suggested_002_anchor=0; do NOT re-correct. ONLY when internal_standard_offset_applied_deg is 0: if the peak is SHARP and SMOOTH (single_peak_FWHM <= ~0.18 AND single_peak_R2 >= ~0.99) but single_peak_center is notably BELOW ~26.50, that is likely a sample-displacement shift, NOT low graphitization - set displacement_suspected=true and suggested_002_anchor=26.54. A BROAD low-angle peak is GENUINE low graphitization - displacement_suspected=false. Be CONSERVATIVE: when unsure, false.
